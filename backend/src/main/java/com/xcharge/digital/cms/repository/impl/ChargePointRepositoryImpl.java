...
public class ChargePointRepositoryImpl implements ChargePointRepository {
    ...
    private @NotBlank(message = "ChargeBox ID is required") String chargeBoxId;
    ...
    @Override
    public ChargePoint.Details getChargeBoxId(String chargeBoxId) {
    ...
    public int addChargePoint(ChargePointForm form) {
        ...
        ChargePoint.Details details = getChargeBoxId(chargeBoxId);
        if(details != null){
            throw new SteveException("'%s' has existed", form.getChargeBoxId());
        }
        return ctx.transactionResult(configuration -> {
            ...
            try {
                ...
                throw new SteveException("Failed to add the charge point with chargeBoxId '%s':" + e.getMessage(),
    ...
    public void updateChargePoint(ChargePointForm form) {
        ...
        ctx.transaction(configuration -> {
            ...
            try {
                ...
                throw new SteveException("Failed to update the charge point with chargeBoxId '%s':" + e.getMessage(),
                        form.getChargeBoxId(), e);
            }
        });
    }
    ...
    public void deleteChargePoint(int chargeBoxPk) {
        ctx.transaction(configuration -> {
            ...
            try {
                ...
                throw new SteveException("'%s' doesn't exist", chargeBoxId, e);
            }
        });
    }
    ...
    public void deleteChargePoint(String chargeBoxId) {
        ChargeBoxRecord cbr = ctx.selectFrom(CHARGE_BOX)
                ...
                .fetchOne();
        if (cbr == null) {
            throw new SteveException("'%s' doesn't exist", chargeBoxId);
        }
        ctx.transaction(configuration -> {
            DSLContext ctx = DSL.using(configuration);
            try {
                Integer chargeBoxPk = cbr.getChargeBoxPk();
                addressRepository.delete(ctx, selectAddressId(chargeBoxPk));
                deleteChargePointInternal(ctx, chargeBoxPk);
            } catch (DataAccessException e) {
